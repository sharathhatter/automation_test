...
import com.bigbasket.mobileapp.handler.payment.PostPaymentResponseHandler;
...
public class PostPaymentProcessor<T> {
    ...
    public void processPaytm(boolean status, HashMap<String, String> paramsMap) {
        new PostPaymentResponseHandler<>(ctx, potentialOrderId, Constants.PAYTM_WALLET, status, orderId)
                .setPayNow(isPayNow)
    ...
    private void validateHdfcPayzappResponse(String pgTxnId, String dataPickupCode,
        ...
        new PostPaymentResponseHandler<>(ctx, potentialOrderId, Constants.HDFC_POWER_PAY,
                true, orderId)
